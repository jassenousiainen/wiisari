...
if ($request == 'GET') {
    ...
    if ((!empty($string)) || (empty($post_groupname)) || (!preg_match('/' . "^([[:alnum:]]| |-|_|\.)+$" . '/ui', $post_groupname)) || ($select_office_name == '1') || (@$tmp_groupname == $post_groupname) || (!empty($string2))) {
        ...
        if (!empty($string)) {
            ...
            echo "            </table>\n";
        } elseif (!preg_match('/' . "^([[:alnum:]]| |-|_|\.)+$" . '/ui', $post_groupname)) {
            echo "            <table align=center class=table_border width=60% border=0 cellpadding=0 cellspacing=3>\n";
            echo "              <tr><td class=table_rows width=20 align=center><img src='../images/icons/cancel.png' /></td><td class=table_rows_red>
