...
if ($request == 'GET' || isset($_POST['errors'])) {
    ...
    if (($_POST['groupID'] != "all") && (!empty($_POST['groupID']))) {
        ...
        if (!isset($group_name)) {
            ...
            $query = tc_query("SELECT * FROM supervises WHERE groupID ='$groupID' AND userID = '$supervisorID'");
            if($query != FALSE){
                $super = mysqli_fetch_row($query);
            }else{
                $super = FALSE;
            }
